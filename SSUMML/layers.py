...
def batch_norm(x, bn_scope, is_training = True):
...
def general_conv2d(inputconv, o_d=64, f_h=7, f_w=7, s_h=1, s_w=1, stddev=0.01,
                   ...
                   relufactor=0, norm_type=None, bn_scope='batch_norm', is_training=True):
    with tf.variable_scope(name):
        ...
        if do_norm:
            if norm_type is None:
                ...
                conv = batch_norm(conv, bn_scope, is_training)
    ...
    with tf.variable_scope(name):
        ...
        if do_norm:
            if norm_type is None:
                ...
                di_conv_2d = batch_norm(di_conv_2d, bn_scope, is_training)
...
def general_deconv2d(inputconv, outshape, o_d=64, f_h=7, f_w=7, s_h=1, s_w=1,
                     ...
                     do_norm=True, do_relu=True, relufactor=0, norm_type=None, bn_scope='batch_norm', is_training=True):
    with tf.variable_scope(name):
